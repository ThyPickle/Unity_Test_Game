...
public class Math : MonoBehaviour
{
    ...
    void Start()
    {
        Invoke("start_troops", 1);
    }

    private void start_troops()
    {
        for (int i = 1; i <= 5; i++)
        {
            Instantiate(player, transform.position, transform.rotation);
        }
        Logic_Manager.Troop_number += 5;
    }
    ...
    void Update()
    {
        switch (Random_Genarator.OP_door_left)
        {
            case 0:                         //  +
                ...
                if (Hitleft == true)
                {
                    for (int i = 1; i <= Random_Genarator.ll_plus; i++)                               
                    {
                        Instantiate(player, transform.position, transform.rotation);
                    }
        ...
        switch (Random_Genarator.OP_door_right)
        {
            case 0:                         //  +
                ...
                if (Hitright == true)
                {
                    for (int i = 1; i <= Random_Genarator.lr_plus; i++)                               
                    {
                        Instantiate(player, transform.position, transform.rotation);
                    }
